...
public class QuizController {
    ...
    void onValidate(ActionEvent event) {
        ...
        boolean isSuccess = percentage >= 60;
    ...
    void onQuit(ActionEvent event) {
        if (currentChapter != null) {
            ChapitreDetailController ctrl = MainController.getInstance().loadViewIntoPage("/ui/chapitre-detail.fxml", (VBox) questionArea.getScene().lookup("#pageCourses"));
            if (ctrl != null) {
                ctrl.setChapter(currentChapter, null);
            }
        } else {
            MainController.getInstance().loadViewIntoPage("/ui/chapitre-list.fxml", (VBox) questionArea.getScene().lookup("#pageCourses"));
        }
    }
}
