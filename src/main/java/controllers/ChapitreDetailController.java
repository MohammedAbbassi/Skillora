...
public class ChapitreDetailController {
    ...
    void onBack(ActionEvent event) {
        try {
            Cours cours = coursService.getById(currentChapter.getIdCours());
            ChapitreListController ctrl = MainController.getInstance().loadViewIntoPage("/ui/chapitre-list.fxml", (VBox) chapterTitleLabel.getScene().lookup("#pageCourses"));
    ...
    void onQuizClick(ActionEvent event) {
        QuizController ctrl = MainController.getInstance().loadViewIntoPage("/ui/quiz-view.fxml", (VBox) chapterTitleLabel.getScene().lookup("#pageCourses"));
        if (ctrl != null) {
    ...
    void onToggleDyslexiaPanel(ActionEvent event) {
        ...
        if (!isVisible) {
            User user = SessionManager.getCurrentUser();
            int userId = (user != null) ? user.getId() : 1;
            applyDyslexiaSettings(dyslexiaService.getPreferences(userId));
        }
    }
